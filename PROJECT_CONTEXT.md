...
### 히어로 → events 물감 전환 (2026-08-10)

메인 비주얼에서 아래로 스크롤하면 티켓 뒤에서 물감(#809975)이 번져 화면을 덮고,
다 덮은 뒤 걷히면서 events 섹션이 시작됩니다.

**변경 파일**

- `index.html` — `.main_visual` 안에 `<div class="visual_bleed" data-visual-bleed>` 추가
- `css/main.css` — `.visual_bleed` / `.visual_bleed.is_full` 추가,
  `.main_visual_ticket` z-index 3 → 5
- `js/main.js` — `initHeroVisualBleed()` 와 `BLEED_*` 상수 추가

**구현**

- 원형 확산이 아니라 **티켓 블롭 에셋(`main_visual_ticket.png`)을 마스크로** 써서
  물감이 번지는 모양을 만듭니다. 새 에셋을 만들지 않았습니다.
- 히어로가 화면 밖으로 빠져나가는 거리(= 히어로 높이)를 진행률로 삼고,
  마스크 크기와 위치를 `--bleed_size` / `--bleed_x` / `--bleed_y` 로 넣습니다.
- 번지는 시작점은 매 프레임 티켓의 화면 좌표에서 다시 구하므로,
  히어로가 스크롤로 밀려 올라가도 물감이 티켓을 따라갑니다.
- z-index: 물감 4, 티켓 5 → 물감이 티켓 "뒤에서" 나옵니다. 정보 바(4)와 copy(2)는 덮입니다.

**진행률 구간** (1440 x 900 기준)

| 진행률 | 상태 |
|---|---|
| 0 ~ 0.08 | 반투명 블롭에서 진한 색으로 배어 나옴 |
| 0.08 ~ 0.5 | 번져 나가 화면을 덮음 |
| 0.65 ~ 0.8 | 꽉 찬 색 (0.65 부터 `is_full` 로 마스크를 벗김) |
| 0.8 ~ 1.0 | 걷히며 events 섹션이 드러남 |

**주요 판단**

- **기존 `initHeroSectionJump()` 을 건드리지 않았습니다.** 휠 한 번에 events 로
  점프하는 기존 동작이 있는데, 물감을 스크롤 위치로만 그리도록 만들어서
  Lenis 가 900ms 동안 부드럽게 이동하는 그 사이에 전환이 재생됩니다.
  스크롤바 드래그나 키보드로 내려도 똑같이 동작합니다.
- **블롭이 오목해서 배율이 크게 필요했습니다.** 대각선의 2 배면 원은 덮이지만
  블롭은 모서리에 홈이 남습니다. 4.5 배로 키우고, 그래도 확실히 하려고
  다 번진 시점부터 `is_full` 로 마스크를 벗겨 통색으로 채웁니다.
- 시작 순간 반투명 블롭 위에 진한 색이 튀지 않도록 진행률 0.08 까지 불투명도를 올립니다.
- `prefers-reduced-motion` 에서는 `display: none` 으로 전환 자체를 끕니다 (AGENTS 10.7).
- `pointer-events: none` 이라 화면을 덮는 동안에도 클릭을 막지 않습니다.

**검증 결과** (캐시 무력화한 임시 사본으로 최신 CSS/JS 강제 로드 후 삭제)

- lint / test / build: 정적 HTML 프로젝트라 `package.json` 이 없어 미실행
- 1440 x 900 — 진행률 0 에서 불투명도 0, 스크롤 시작과 함께 번지기 시작
- 번지는 시작점이 티켓 중심(1192.7, ...)과 일치하고, 스크롤에 따라 y 가 따라 올라감
- `mask-position` 이 `중심 - 크기/2` 로 정확히 계산됨 (1192.7 - 943.35 = 249.35 확인)
- 진행률 0.24 / 0.50 / 0.68 화면 캡처 — 티켓 뒤에서 블롭이 번져 나와 화면을 덮는 것 확인,
  티켓 콘텐츠는 물감 위에 그대로 남음
- 진행률 0.50 에서 화면 전체가 덮임
