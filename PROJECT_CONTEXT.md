...
### N Gift Shop 카테고리 필터 스크롤 복구 (2026-08-10)

Souvenirs와 Toys 필터 선택 후 페이지 스크롤이 멈출 수 있는 문제를 수정했습니다.
...
- 필터 선택 시 페이지 상단으로 이동하는 기존 동작은 유지합니다.
- Lenis가 활성화된 경우 `scrollTo(0)`만 호출하고, `stop()` 후 다음 프레임에서 `start()`하던 로직은 제거했습니다.
- Lenis가 없는 환경에서는 기존 네이티브 `window.scrollTo(0, 0)`을 사용합니다.
- 상품 카드 12개의 전체 콘텐츠를 `href="#"` 링크로 감싸 클릭 가능하게 구성했습니다.
- 링크 요소가 기존 카드 레이아웃과 색상을 유지하도록 Gift Shop 전용 스타일을 추가했습니다.

**검증 결과**

- `node --check js/n_gift_shop.js`: 통과
- 상품 카드와 링크 개수: 각각 12개 확인
- 변경 범위: Gift Shop 전용 HTML, CSS, JavaScript와 이 기록 문서만 변경
- 실제 브라우저 휠/터치 재현: 로컬 브라우저 연결 제한으로 미실행
